...
int main()
{
    ...
    while (1) {
        ...
        if (type == "Circle") {
            ...
            std::pair<double, double> point
                    = figures.back().get_pair_of_numbers(
                            input, &position, ' ', ',');
            if (point.second == -1) {
                figures.erase(prev(figures.end()));
                std::cout << "Incorrect point.\nFollow this type: Circle(x y, "
                             "r)\n";
                continue;
            }
    ...
    // for (auto& i : figures) {
    // 	std:: cout << i.get_coords().first << " " << i.get_coords().second << "
    // " << i.get_radius() << std::endl;
    // }
    // std::cout << figures.size() << std::endl;
    for (int i = 0; i < (int)figures.size(); ++i) {
        for (int j = i + 1; j < (int)figures.size(); ++j) {
            if (figures[i].get_type() == "Circle"
                and figures[j].get_type() == "Circle") {
                std::pair<double, double> a = figures[i].get_coords(),
                                          b = figures[j].get_coords();
                // std:: cout << figures[i].get_radius() +
                // figures[j].get_radius() << std::endl; std::cout <<
                // dist(a.first,a.second,b.first,b.second) << std::endl;
                if (figures[i].get_radius() + figures[j].get_radius()
                    >= dist(a.first, a.second, b.first, b.second)) {
                    crossed_figures.emplace_back(
                            figures[i].get_index(), figures[j].get_index());
                }
            }
        }
    }
